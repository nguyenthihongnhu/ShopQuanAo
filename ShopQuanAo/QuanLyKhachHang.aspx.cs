...
namespace ShopQuanAo
{
    public partial class QuanLyKhachHang : System.Web.UI.Page
    {
        ...
        protected void gvKhachHang_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            int maKH = int.Parse(gvKhachHang.DataKeys[e.RowIndex].Value.ToString());
            BUS_KhachHang busKH = new BUS_KhachHang();
            if (busKH.IsAdmin(maKH))
            {
                WebMsgBox.Show("Không thể xoá tài khoản Admin !");
                return;
            }

            if (busKH.DaCoHoaDon(maKH))
            {
                WebMsgBox.Show("Không thể xoá tài khoản đang có đơn đặt hàng !");
                return;
            }
            ...
            if (Session["Username"] != null)
            {
                ...
                KhachHang khachHang = busKH.LayThongTinKhachHang(userName);
                if (khachHang.MAKH == maKH)
                {
                    WebMsgBox.Show("Không thể xoá tài khoản hiện đang đăng nhập !");
                    return;
                }
